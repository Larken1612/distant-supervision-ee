    ...
    for data in json_list:
        ...
        for trigger in data.get("event_triggers", []):
            ...
            if roles:
                ...
                event_arguments[event_type][tuple(roles_list)] += (
                    1  # Convert back to tuple for dict key
                )
                ...
                for role, entity_counts in type_count.items():
                    ...
                    for entity_type, count in entity_counts.items():
                        if (
                            entity_type
                            not in event_type_count[event_type][tuple(roles_list)][role]
                        ):
                            event_type_count[event_type][tuple(roles_list)][role][
                                entity_type
                            ] = 1
        ...
        arguments_list = [
            {
                ...
                "count": value,
                "type_count": {
                    role: dict(entity_counts)
                    for role, entity_counts in event_type_count[event_type][key].items()
                },
            }
    ...
    for event_type in event_dict:
        try:
            threshold = argument_threshold(
                [arg["count"] for arg in event_dict[event_type]["arguments"]]
            )
        ...
        event_dict[event_type]["arguments"] = [
            arg
            for arg in event_dict[event_type]["arguments"]
        ...
        for event in record.get("event_triggers", []):
            ...
            if event_type in result_graph:
                event_args = sorted(
                    [
                        arg["role_type"]
                        for arg in record.get("event_arguments", [])
        ...
        entity_mentions.append(
            {
                ...
                "end_char": input_json["text"].find(entity["text"])
                + len(entity["text"]),
                "mention_type": "UNK",
            }
        )
        ...
        arguments = [
            {
                ...
                "text": next(
                    e["text"]
                    for e in input_json["entity_mentions"]
                    if e["entity_id"] == arg["entity_id"]
                ),
                "role": arg["role_type"],
            }
            for arg in input_json["event_arguments"]
            if arg["trigger_id"] == event["trigger_id"]
        ]
        event_mentions.append(
            {
                ...
                "trigger": {
                    ...
                    "end_char": input_json["text"].find(event["text"])
                    + len(event["text"]),
                },
                "arguments": arguments,
            }
...
file_path = "result/filtered_matched_date.json" # choose 1 file in result/
